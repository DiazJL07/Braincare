...
// Verificar si el usuario es administrador o psicólogo
exports.adminOrPsychologist = (req, res, next) => {
  if (req.user && (req.user.role === 'admin' || req.user.role === 'psychologist')) {
    return next();
  }
  return res.status(403).render('error', {
    message: 'Acceso denegado. Se requieren permisos de administrador o psicólogo.'
  });
};
...
exports.checkUser = async (req, res, next) => {
  ...
  if (token && token !== '') {
    try {
      ...
      const currentTime = Math.floor(Date.now() / 1000);
      if (decoded.exp && decoded.exp < currentTime) {
        // Token expirado, eliminar cookie y continuar
        res.cookie('jwt', '', {
          httpOnly: true,
          expires: new Date(0),
          path: '/'
