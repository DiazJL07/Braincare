// Variables globales
let sessionId = localStorage.getItem('brainbot_session_id') || null;
let conversation = [];
...
async function clearConversation() {
  ...
  try {
    // Usar la URL del servidor de IA
    const apiUrl = 'http://localhost:5001/api/conversation';
  ...
  try {
    ...
    if (data.error) {
      ...
      if (data.session_id && !sessionId) {
        ...
        localStorage.setItem('brainbot_session_id', sessionId);
      }
