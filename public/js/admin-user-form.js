...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Estado de carga y bloqueo para evitar dobles envíos
        submitBtn.classList.add('loading');
        submitBtn.disabled = true;
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Creando...';
        // Permitir envío nativo al servidor (creación automática)
    });
  ...
  const dangerAlert = document.querySelector('.user-alert.user-alert-danger');
  if (dangerAlert) {
        dangerAlert.style.opacity = '1';
        setTimeout(() => {
            dangerAlert.style.transition = 'opacity 0.4s ease';
            dangerAlert.style.opacity = '0';
            setTimeout(() => { if (dangerAlert && dangerAlert.parentNode) { dangerAlert.parentNode.removeChild(dangerAlert); } }, 400);
        }, 4000);
  }
  // Auto-guardado de rol al cambiar
  const roleSelect = document.getElementById('role');
  const nameInput = document.getElementById('name');
  const emailInput = document.getElementById('email');
  if (roleSelect && form && form.action && form.querySelector('input[name="_method"][value="PUT"]')) {
    roleSelect.addEventListener('change', async function(){
      try {
        ...
        let alert = document.querySelector('.user-alert.user-alert-success');
        if (!alert) {
          alert = document.createElement('div');
          alert.className = 'user-alert user-alert-success';
          alert.setAttribute('role','alert');
          alert.innerHTML = '<i class="fas fa-check-circle"></i> <span>Rol actualizado correctamente</span>';
          const panel = document.querySelector('.user-form-panel');
          if (panel) panel.insertBefore(alert, panel.firstChild);
        } else {
          const span = alert.querySelector('span');
          if (span) span.textContent = 'Rol actualizado correctamente';
        }
        alert.style.opacity = '1';
        setTimeout(() => {
          alert.style.transition = 'opacity 0.4s ease';
          alert.style.opacity = '0';
          setTimeout(() => { if (alert && alert.parentNode) { alert.parentNode.removeChild(alert); } }, 400);
        }, 2500);
      } catch (e) {
      } finally {
