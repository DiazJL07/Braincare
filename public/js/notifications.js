...
if (typeof window.NotificationSystem === 'undefined') {
  ...
  animateBell() {
    ...
    if (bell) {
      const icon = bell.querySelector('.fas.fa-envelope') || bell.querySelector('.fas.fa-bell');
      if (icon) {
  ...
  async renderModalContent() {
    ...
    if (this.isAdmin) {
      try {
        const r = await fetch('/reports/pending-count');
        if (r.ok) {
          const d = await r.json();
          adminSummary = `<div class="notifications-summary"><div><i class="fas fa-flag"></i> Reportes pendientes: ${d.count}</div></div>`;
        }
      } catch (_) {}
    }
  ...
  handleNotificationClick(notificationId) {
    ...
    if (notification && !notification.read) {
      ...
      if (this.isAdmin && notification && notification.relatedReport) {
        window.location.href = `/admin/reports/${notification.relatedReport}`;
      }
    }
  }
  ...
  getNotificationIcon(type) {
    switch (type) {
      ...
      case 'new_session': return 'fas fa-calendar-check text-success';
