...
router.get('/users/:id', async (req, res) => {
  try {
    ...
    res.render('admin/user-detail', { 
      user, 
      userStats,
      recentActivity: recentActivity.slice(0, 10) // Mostrar solo las 10 más recientes
    });
  } catch (error) {
...
router.post('/users/:id', async (req, res) => {
  try {
    ...
    const nameChanged = currentUser.name !== name;
    ...
    // Si el nombre cambió, actualizar todas las referencias
    if (nameChanged) {
      const Foro = require('../models/Foro');
      const Comment = require('../models/Comment');
      const Report = require('../models/Report');
      
      // Actualizar referencias en publicaciones del foro
      ...
      // Actualizar referencias en reportes (como reportado)
      await Report.updateMany(
        { 'reportedUser.name': currentUser.name },
        { $set: { 'reportedUser.name': name } }
      );
      
      // Actualizar referencias en reportes (como quien reporta)
      await Report.updateMany(
        { 'reportedBy.name': currentUser.name },
        { $set: { 'reportedBy.name': name } }
      );
    }
