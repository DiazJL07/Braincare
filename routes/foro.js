...
const storage = cloud.enabled ? multer.memoryStorage() : multer.diskStorage({
  destination: (req, file, cb) => {
    const dest = path.join(PUBLIC_DIR, 'img', 'foro');
...
router.post('/admin', protect, admin, upload.single('image'), async (req, res) => {
  try {
    ...
    let up;
    if (req.file && cloud.enabled) {
      up = await cloud.uploadImage(req.file.buffer, { folder: 'foro' });
    }
    const data = {
      title,
      topic,
      content,
      author: req.user._id,
      ...(req.file && (cloud.enabled
        ? { image: up.url, imagePublicId: up.public_id }
        : { image: '/img/foro/' + req.file.filename }))
    };
...
router.post('/admin/:id', protect, admin, upload.single('image'), async (req, res) => {
  try {
    ...
    if (req.file) {
      if (cloud.enabled) {
        const foroPrev = await Foro.findById(req.params.id);
        const up = await cloud.uploadImage(req.file.buffer, { folder: 'foro' });
        update.image = cloud.buildImageUrl(up.public_id);
        update.imagePublicId = up.public_id;
        const oldId = foroPrev && (foroPrev.imagePublicId || cloud.extractPublicIdFromUrl(foroPrev.image));
        if (oldId) await cloud.deleteResource(oldId, 'image');
      } else {
        update.image = '/img/foro/' + req.file.filename;
      }
    }
...
router.post('/', protect, requireNotBanned, upload.single('image'), async (req, res) => {
  try {
    ...
    if (req.file && cloud.enabled) {
      const up = await cloud.uploadImage(req.file.buffer, { folder: 'foro' });
      data.image = cloud.buildImageUrl(up.public_id);
