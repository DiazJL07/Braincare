...
router.get('/profile', async (req, res) => {
  try {
    ...
    // Verificar estado de solicitud de psicólogo
    ...
    const latestRequest = await PsychologistRequest.findOne({ user: req.user._id })
      .sort({ createdAt: -1 })
      .lean();
    res.render('user/profile', { user, latestPsychRequest: latestRequest });
  } catch (error) {
...
router.post('/profile/cover', uploadCoverSafe, async (req, res) => {
  try {
    ...
    if (req.user.coverImage && req.user.coverImage !== '/img/default-cover.svg') {
      const oldImagePath = path.join(__dirname, '../public', req.user.coverImage);
      if (fs.existsSync(oldImagePath)) {
        fs.unlinkSync(oldImagePath);
      }
    }
